...
export default function PlayerProfileCard({
  ...
  return (
    <div
      className={`w-full rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-all dark:border-[#282828] dark:bg-[#1c1c1c] dark:shadow-none ${className}`}
    >
      {/* Section Title */}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3.5 min-w-0">
          ...
          <div className="relative shrink-0">
            <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-slate-100 font-semibold text-slate-700 ring-2 ring-[#3ecf8e]/30 dark:bg-[#262626] dark:text-zinc-100">
              {avatarUrl ? (
            ...
            {/* Activity status dot — overlaps the photo edge */}
            <span
              className={`absolute -bottom-0.5 -right-0.5 z-10 h-3.5 w-3.5 rounded-full border-2 border-white dark:border-[#1c1c1c] ${status.dot}`}
              title={status.label}
            />
          </div>
